...
function mostrarDatosClima() {
  const apiKey = '178cdec44fce43a4fadeabb33861ee87' 
  const url = `https://api.openweathermap.org/data/2.5/weather?q=Buenos%20Aires,AR&appid=${apiKey}&units=metric`
  ...
  fetch(url)
    ...
    .then(data => {
      ...
      contenido.innerHTML = `Ubicacion Del Local: ${name}, Caballito. <br> Temperatura actual: ${temp}°C, Temperatura máxima: ${temp_max}°C, Temperatura mínima: ${temp_min}°C`

      const resultadoClima = document.getElementById('resultado-clima')
      resultadoClima.appendChild(contenido)

    })
    .catch(error => console.error(error)) // .CATCH RECOLETA MIS HERMOSOS ERRORES 
}
...
function agregarAlCarritoClicked(event){
  ...
  hacerVisibleCarrito()
  localStorage.setItem('carrito', 'Se agrego el producto al localstorage') 
}
